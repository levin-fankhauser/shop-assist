...
export default function OrderFormDialog({
  ...
  return (
    <>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-30 flex items-start justify-center overflow-y-auto bg-slate-950/80 px-4 py-10">
          <div className="w-full max-w-2xl rounded-2xl border border-white/15 bg-slate-900/95 p-6 shadow-2xl shadow-slate-950/60">
            <div className="flex items-start justify-between gap-3">
              ...
              <button
                className="rounded-lg border border-white/10 p-2 text-slate-300 transition hover:border-rose-300/50 hover:text-white"
            ...
            <form className="mt-6 space-y-4" onSubmit={submit}>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    type="button"
                    className="inline-flex items-center gap-2 rounded-lg border border-white/10 px-3 py-1.5 text-sm font-medium text-cyan-100 transition hover:border-cyan-300/60"
                    onClick={() =>
                      setProducts((current) => [
                        ...current,
                        { ...emptyProduct },
                  ...
                  {products.map((product, index) => (
                    ...
                    >
                      <div className="grid grid-cols-1 gap-3 sm:grid-cols-[2fr_1fr_auto] sm:items-center">
                        <input
                          className="w-full rounded-lg border border-white/10 bg-slate-900/60 px-3 py-2 text-slate-100 outline-none transition focus:border-cyan-300/70"
                          value={product.name}
                          onChange={(e) =>
                            setProducts((current) =>
                              current.map((item, i) =>
                        ...
                        <button
                          type="button"
                          className="inline-flex items-center justify-center rounded-lg border border-white/10 px-3 py-2 text-sm text-slate-200 transition hover:border-rose-300/50 hover:text-rose-100"
                          onClick={() =>
                            setProducts((current) =>
                              current.length === 1
                                ? current
                      ...
                      <textarea
                        className="mt-3 w-full rounded-lg border border-white/10 bg-slate-900/60 px-3 py-2 text-sm text-slate-100 outline-none transition focus:border-cyan-300/70"
              ...
              <div className="flex items-center justify-end gap-3">
                ...
                <button
                  type="submit"
                  className="inline-flex items-center gap-2 rounded-lg bg-cyan-400 px-5 py-2 text-sm font-semibold text-slate-950 transition hover:bg-cyan-300 disabled:cursor-not-allowed disabled:opacity-70"
